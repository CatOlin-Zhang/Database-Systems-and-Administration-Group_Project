...
#更新订单总价格
def update_order_total(order_id, total_price):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
# 创建交易记录
def create_transaction(order_id, vendor_id, pay_amount):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
#根据订单ID统计各供应商金额
def get_vendor_totals_by_order(order_id):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
# 删除订单关联的交易记录
def delete_order_transactions(order_id):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
#将订单状态更新为待发货
def update_order_status_to_shipping(order_id):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
...
#将订单状态更新为已完成
def update_order_status_to_completed(order_id):
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
